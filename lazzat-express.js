...
function loadAllMenus() {
  return Promise.all([
    getMenuFromRestaurant('Salim'),
    getMenuFromRestaurant('İbad'),
    getMenuFromRestaurant('Elmin'),
  ])
...
export function checkRestaurant(name) {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      if (name === 'İbad') reject(new Error(`${name} is closed`));
...
function openRestaurants() {
  return Promise.allSettled([
    checkRestaurant('Salim'),
    checkRestaurant('İbad'),
    checkRestaurant('Elmin'),
...
5)const cache = { home: 'Akhmedli', work: 'iwsiz' };
